...
public class IfStatement implements IStatement {
    ...
    public IValue execute(ExecutionContext context) {
        ...
        if (executed instanceof BooleanValue) {
            if (((BooleanValue) executed).getValue()) {
                successBody.execute(context);
            } else {
                if (failBody != null) {
                    failBody.execute(context);
                }
            }
        } else {
            throw new RuntimeException("Condition under if should be a boolean value");
        }
        return null;
    }
}
